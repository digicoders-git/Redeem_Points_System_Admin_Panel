    ...
    <div className="min-h-screen bg-[#F5F7FA] font-sans pb-24">
      ...
      <div className="bg-[#0f4089] rounded-b-[32px] px-5 pt-10 pb-10 relative overflow-hidden shadow-lg mb-5">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div>
            <h1 className="text-white font-bold text-xl tracking-wide mb-0.5">Terms & Conditions</h1>
            <p className="text-white/70 text-sm">Manage policy points</p>
          </div>
      ...
      <div className="px-4">
        ...
        <form onSubmit={add} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-5">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 border-2 border-gray-100 bg-[#F5F7FA] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#0f4089]/30 transition-colors"
            />
            <button
              type="submit"
              disabled={adding || !newText.trim()}
              className="bg-[#0f4089] text-white px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-1.5 disabled:opacity-50 active:scale-95 transition shrink-0"
            >
        ...
        {loading ? (
          <div className="flex justify-center py-16">
            <Loader2 size={30} className="animate-spin text-[#0f4089]" />
          </div>
        ) : (
          <div className="space-y-2.5">
            ...
            {terms.map((t, i) => (
              <div key={t._id} className={`bg-white rounded-2xl border shadow-sm p-4 transition ${!t.isActive ? "opacity-50 border-gray-100" : "border-gray-100"}`}>
                {editId === t._id ? (
                  <div className="flex gap-2 items-start">
                    <textarea
                      ...
                      rows={2}
                      className="flex-1 border-2 border-[#0f4089]/30 bg-[#F5F7FA] rounded-xl px-3 py-2 text-sm focus:outline-none resize-none"
                      autoFocus
                    />
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex gap-1.5 shrink-0">
                      <button onClick={() => toggle(t)} disabled={actionId === t._id} className={`w-8 h-8 rounded-lg flex items-center justify-center transition ${t.isActive ? "bg-green-50 text-green-600 hover:bg-green-100" : "bg-gray-100 text-gray-400 hover:bg-gray-200"}`}>
                        {actionId === t._id ? <Loader2 size={13} className="animate-spin" /> : t.isActive ? <ToggleRight size={15} /> : <ToggleLeft size={15} />}
                      </button>
                      <button onClick={() => { setEditId(t._id); setEditText(t.text); }} className="w-8 h-8 rounded-lg bg-blue-50 text-[#0f4089] flex items-center justify-center hover:bg-blue-100 transition">
                        <Pencil size={13} />
                      </button>
                      <button onClick={() => remove(t._id)} disabled={actionId === t._id} className="w-8 h-8 rounded-lg bg-red-50 text-red-500 flex items-center justify-center hover:bg-red-100 transition">
